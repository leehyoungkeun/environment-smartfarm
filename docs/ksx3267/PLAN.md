...
### Phase 7 — 116 검정 갭 (3일) — **완료 2026-08-30** (추출·1분 조회·구동기 1분 스냅샷) + **2026-09-14** 연결 해제·재연결 자가시험(`116-재연결`)·30일 창 감시 지표/경보
...
| 문서 | 위치 | 상태 (2026-09-14) |
|---|---|---|
| 규격 및 성능설명서 (= 제품 사양서·연동장비표) | `manual/규격및성능설명서.md` v0.2 | 기술 사양 완료, 1호 실측값 반영. 남은 〔〕 = 형식명·치수·부품 조달 정보 |
| 제품사용설명서 (필수 4항목) | `manual/사용설명서.md` | 조작·안전·점검 완료. 남은 〔〕 = 보증기간·A/S 접수·부품 판매·사후관리업소·사업자 정보 |
| 지원 선언서 | 이 문서 §0 | 완료 |
| 자체 시험 성적서 | `self-test/성적서.md` + `self-test/evidence-*` | 시뮬레이터 상대 완료. 실 노드 교차는 HW 후 |
| 설치 SOP (커넥터·핀·종단) | `manual/사용설명서.md` §2·§9 | 완료 |
| 시험기관 사전 문의서 | `koat-inquiry.md` | 작성 완료, **발송 대기** |
| 신청서 2종 (성능시험 + 국가표준시험) | KOAT 양식 | 사업자등록증과 함께 회사 작성 |
...
## 6. 완료 기준
- [x] 시뮬레이터 상대 §5.4/5.5 전 시나리오 통과 — 8/30 표 4 7/7, 9/4 §5.5.2 13/13·§5.5.3 14/14 완전판
- [ ] 실 노드 1대 교차 통과 — **HW 대기** (표준 포트 FTDI 는 1호에 장착·심링크 확인됨, 실 노드 미확보)
- [x] 게이트 테스트에 codec·discovery·execute_control 분기·프록시 포함, 변이 검사 통과 — 백엔드 370/370 (2026-09-14)
- [x] 운영 경로(Waveshare/XY-MD02) 회귀 0 (기존 테스트 전부 통과)
- [~] 문서 3종 + 증적 — 기술 내용·증적 완료. 〔〕 는 시험 기체 실측·회사 A/S 정보 기입 대기

## 7. 남은 것 — 소프트웨어 밖 (2026-09-14)

소프트웨어·문서로 할 수 있는 일은 끝났다. 아래는 사람·물건·시간이 필요하다.

| 할 일 | 누가 | 조건·주의 |
|---|---|---|
| KOAT 사전 문의 발송 | 케이그린텍 | `koat-inquiry.md` 그대로. **1~3번 답이 시험 기체 구성과 반복 횟수를 정한다** — 가장 먼저 |
| 관제 방식 신고 개수 결정 | 케이그린텍 | 신고 수 = 연결 해제 시험 반복 배수. 문의 3번 답 후 |
| 시험 전용 기체 제작 | 케이그린텍 | 운영 중인 1호는 입고 불가. 함체 치수 실측 → 규격서 〔〕 기입 |
| 실 노드 1대 확보 + 교차 시험 | 케이그린텍 + Claude | `selftest_sps7466.py` 를 `--port /dev/smartfarm-485-std` 드라이버로 재실행 |
| 회사 A/S 정보 기입 | 케이그린텍 | 사용설명서 §11~13 (보증기간·접수처·부품 판매·사후관리업소·사업자) |
| **30일 데이터 → 입고일** | 시간 | 9/12 정전·9/13 이동으로 창이 끊겨 **9/14 부터 다시 셈 → 가장 빠른 입고 10/14**. 지표 `smartfarm_ks_data_window_days`·`_ready_in_days`, 끊기면 경보 `KsDataWindowBroken`. 입고 기체로 옮길 때는 데이터도 함께 |
| 전원 차단 30분 드릴 1회 | 케이그린텍 | 30분 정전 = 30행 = 그날 손실 2.08% 로 **3% 이내라 창을 끊지 않는다.** 단 복구가 13분 넘게 늦어지면(합계 43분↑) 끊긴다 — 드릴 날은 사람이 붙어 있을 것 |
