...
function handleClientMessage(ws, msg) {
  switch (msg.type) {
    ...
    case "system:query": {
      // 시스템 상태 조회 (Category A) — 정확한 PM2 process status
      const farmId = msg.farmId || ws.farmId;
      if (!ws.isSystemWide && farmId !== ws.farmId) {
        ws.send(JSON.stringify({ type: "error", message: "권한 없음" }));
        return;
      }
      const sent = mqttService.publishSystemQuery(farmId);
      ws.send(JSON.stringify({
        type: "system:query:ack",
        farmId,
        sent,
        mqtt: mqttService.isConnected(),
      }));
      const cached = mqttService.getSystemStatus(farmId);
      if (cached) {
        ws.send(JSON.stringify({ type: "system:status", farmId, data: cached, cached: true }));
      }
      break;
    }
...
function setupMqttBridge() {
  ...
  // 시스템 상태 → 해당 농장 WebSocket 클라이언트로 전달
  mqttService.on("system:status", ({ farmId, data }) => {
    broadcast(farmId, { type: "system:status", farmId, data });
  });
