...
case "${1:-status}" in
  up)
    ...
    if [ -n "$BE" ]; then
      ...
      for f in "$BE"/prisma/migration-relay-status.sql "$BE"/prisma/migration-device-positions.sql "$BE"/prisma/migration-kakao-links.sql "$BE"/prisma/migration-actuator-status.sql "$BE"/prisma/migration-ks-sensor-status.sql "$BE"/prisma/migration-ks-compression.sql; do
        [ -f "$f" ] || continue
