...
0. **거버너는 발열 조절 수단이 아니다. 팬 부재가 전부다** (2026-09-14 1호 A/B 실측).
   유휴 58~60도인데 CPU 사용률은 1% 미만. `time_in_state` **누적값**을 보고 "40% 를 최고
   클럭에서 보낸다"고 판단해 `schedutil` 로 바꿨는데, 같은 조건 300초 A/B 로 재보니 반대였다.

   | 거버너 | 1.5GHz | 2.4GHz | 평균온도 |
   |---|---|---|---|
   | ondemand (기본) | 87.1% | **0%** | 55.2도 |
   | schedutil | 63.6% | 33.2% | 56.3도 |

   `ondemand` 는 진짜 유휴일 때 최고 클럭을 아예 건드리지 않는다. 원복했다.
   **함정: 누적 `time_in_state` 로 판단하지 말 것** — 부팅 구간과 진단 도구 실행분이 섞인다.
   반드시 두 스냅샷 차분으로, 측정 중에는 아무것도 돌리지 말 것.
   (8/30 메모의 "ondemand 가 60초 중 35초를 2.4GHz" 는 chromium 이 애니메이션으로 CPU 를
   태우던 때의 값이라 8/31 수정 이후로는 유효하지 않다.)
   근본 해결은 액티브 쿨러 장착이다 — 1호는 아직 미장착.
