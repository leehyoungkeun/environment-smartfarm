...
# PM2 단일 소스 = ecosystem.config.js (3개 entry: node-red + smartfarm-rpi + smartfarm-system)
# 트랩 13 fix (2026-05-08): 옛 system-server.js 시작 실패로 dump.pm2 가 1개로 reset 되던 문제
sudo -u "$SMARTFARM_USER" bash -c "
  pm2 delete all 2>/dev/null || true
  pm2 start ~/smartfarm/ecosystem.config.js
  pm2 save
"
