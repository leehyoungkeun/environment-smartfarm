    ...
    function startScan() {
      ...
      scanner.start(
        ...
        (decodedText) => {
          ...
          const match = decodedText.match(/device\/([A-Z0-9]{6})/i);
...
router.post('/apply', async (req, res) => {
  ...
  try {
    ...
    const { farmId, farm, mqttClientId, serverUrl } = setupData.data;
    ...
    // 3. Node-RED flows.json 탭 환경변수 일괄 변경
    try {
